...
import { User, Consultation, Message } from '@/lib/types';
...
export default function ConsultantChatPage() {
  ...
  useEffect(() => {
    ...
    auth.verify(authToken).then(({ user: verifiedUser, error }) => {
      ...
      // Fetch assigned chat consultations
  ...
  useEffect(() => {
    ...
    api.get<Message[]>(`/messages?consultationId=${selectedConsultation.id}`, token.current).then((result) => {
      if (result.data) {
      ...
      <div className="flex h-[calc(100vh-120px)]">
        ...
        <div className="w-80 border-r border-gray-200 bg-white flex flex-col">
          <div className="p-4 border-b border-gray-200">
            <h2 className="text-lg font-bold text-gray-900">Assigned Chats</h2>
          </div>
          <div className="flex-1 overflow-y-auto">
            {consultations.length === 0 ? (
              <div className="p-4 text-center text-gray-500">
                <MessageSquare className="w-8 h-8 mx-auto mb-2 text-gray-400" />
                <p>No chat consultations assigned yet</p>
              </div>
            ) : (
              <div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-gray-900 truncate">
                          {consultation.client?.fullName || `Client #${consultation.clientId.slice(0, 8)}`}
                        </p>
            ...
            <>
              <div className="p-4 bg-white border-b border-gray-200">
                <h2 className="text-lg font-bold text-gray-900">
                  Chat with {selectedConsultation.client?.fullName || 'Client'}
                </h2>
