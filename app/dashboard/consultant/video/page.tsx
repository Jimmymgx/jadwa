...
export default function VideoConsultationsPage() {
  ...
  useEffect(() => {
    ...
    auth.verify(token).then(({ user: verifiedUser, error }) => {
      ...
      // Fetch video consultations assigned to this consultant
      api.get<Consultation[]>('/consultations/my?type=video', token).then((result) => {
        if (result.data) {
  ...
  const handleUpdateStatus = async (consultationId: string, newStatus: string) => {
    ...
    const result = await api.put<Consultation>(
    ...
    if (result.data) {
      setConsultations((prev) =>
        prev.map((c) => (c.id === consultationId ? result.data! : c))
      );
    }
  };
    ...
    >
      <div className="space-y-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage your assigned video consultation sessions</p>
        </div>

        {/* Consultations List */}
        <div className="card">
          <div className="p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Assigned Consultations</h2>
            {consultations.length === 0 ? (
              <div className="text-center py-12">
                <Video className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                <p className="text-gray-600">No video consultations assigned yet</p>
                <p className="text-sm text-gray-500 mt-2">
                  You'll see consultations here when clients book sessions with you
                </p>
              </div>
            ) : (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 text-sm text-gray-600 mt-1">
                              ...
                              <span>Client: {consultation.client?.fullName || 'N/A'}</span>
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-4 mt-3 text-sm text-gray-600">
                          ...
                          <span className="font-semibold text-gray-900">
                            {Number(consultation.price).toFixed(2)} SAR
                          </span>
                        </div>
                        {consultation.notes && (
                      ...
                      <div className="flex flex-col items-end gap-2">
                        ...
                        {consultation.status === 'confirmed' && (
                          <button
                            onClick={() => handleUpdateStatus(consultation.id, 'in_progress')}
